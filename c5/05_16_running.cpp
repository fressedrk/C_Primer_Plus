...
int main(void)
{	double distk, distm; //跑过的距离(以公里和英里为单位)
	double rate;//平均速度(英里/时)
	int min, sec; //跑步用时(分钟和秒为单位)
	int time;//跑步用时(秒为单位)
	double mtime;//跑1英里的时间(秒为单位)
	int mmin, msec; //跑1公里的时间(分钟和秒为单位)
	...
	scanf("%d", &sec);
	time = S_PER_M * min + sec;//时间换成秒
	distm = M_PER_K * distk;//公里换成英里
	rate = distm / time * S_PER_H; //英里/小时
	mtime = (double)time / distm; //跑1英里所用时间
